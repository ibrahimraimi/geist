...
Copy [app.css](https://github.com/ibrahimraimi/geist/blob/main/src/app.css) to `src/app.css`

Copy [tailwind.config.ts](https://github.com/ibrahimraimi/geist/blob/main/tailwind.config.ts) to `tailwind.config.ts`
...
```bash
pnpm dlx jsrepo init --project --repos github/ibrahimraimi/geist
```
...
```diff
{
	...
	"repos": ["github/ibrahimraimi/geist"],
	"includeTests": false,
	"watermark": true,
	"formatter": "prettier",
	"paths": {
